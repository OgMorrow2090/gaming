...
RESEARCH_SYSTEM = (
    ...
    "bullet points, and no preamble such as 'I'll look that up'.\n\n"
    "Include #CRAFT FOR GOLD# ONLY when the item is a crafting material or a "
    "craftable item (e.g. food, ingredients, refined materials, gear). In it, "
    "explain the most profitable way to turn this item into Trading Post gold: "
    "name the best recipe(s) to craft from it (or to craft it into), the "
    "crafting profession and discipline level needed, the key ingredients, and "
    "whether crafting then selling beats just selling the raw item on the TP — "
    "with concrete item names and the buy-ingredients-vs-sell-output profit "
    "reasoning. List each candidate recipe on its own line, most profitable "
    "first, without bullet characters. Skip this section entirely for items "
    "that are not materials or craftables.\n\n"
    "Output nothing but the headers and their prose."
)
