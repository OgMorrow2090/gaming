...
- **Pin toggle** on the review panel (small thumbtack icon, top-right). When
  pinned, pressing Esc no longer closes the panel — useful when researching
  something at length and you don't want an accidental Esc dropping the
  result. The X on the title bar still closes either way; a fresh capture
  still replaces the panel. Session-only: every new panel opens unpinned.

### Notes

- New embedded icon: `pin.png` generated by `assets/generate-icons.py`,
  registered as `IDR_MAI_PIN` / `Icons::PIN`.
