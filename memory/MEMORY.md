...
- [GW2 Wine DPI is per-profile and FIXED](per-profile-fixed-dpi.md) — Apple TV profile always 0x90 (144 DPI), Deck profile always 0x60 (96 DPI). Never wire dynamic per-mode DPI swap into Sunshine prep-cmd. Profiles are dedicated to specific stream targets, so LogPixels is fixed per profile.
