...
- [KEYPAD_MINUS is not a valid Steam Input key token](steam-input-invalid-key-tokens.md) — invalid tokens parse fine but silently fail to bind; a chord "does nothing" — check `~/.local/share/Steam/logs/controller.txt` for "Failed to create digital binding"
