...
- [GW2 Wine DPI — historical per-profile + current main-install state](per-profile-fixed-dpi.md) — **ARCHIVED 2026-05-12** for the multi-profile mappings; main install now at 0xC0 (200%) + MouseSensitivity 16 for couch play on LG. Rule still applies: never wire dynamic per-mode DPI swap into Sunshine prep-cmd.
